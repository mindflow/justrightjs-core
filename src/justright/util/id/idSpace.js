import { MacUtils, RadixUtils, StringUtils } from "coreutil_v1";
...
export class IdSpace {
    ...
    static parse(idSpaceString) {
        if (idSpaceString == null || idSpaceString.length < IdSpace.ID_SPACE_STRING_WIDTH || !RadixUtils.isValidRadixString(idSpaceString)) {
        ...
        const mac = RadixUtils.fromRadixString(macString);
        const epochSeconds = RadixUtils.fromRadixString(epochSecondsString);
        const counter = RadixUtils.fromRadixString(counterString);
     ...
     */
    toString() {
        const macString = StringUtils.leftPad(RadixUtils.toRadixString(this.mac), IdSpace.HW_STRING_PART_WIDTH, '0');
        const epochSecondsString = StringUtils.leftPad(RadixUtils.toRadixString(this.epochSeconds), IdSpace.EPOCH_SECONDS_STRING_PART_WIDTH, '0');
        const counterString = StringUtils.leftPad(RadixUtils.toRadixString(this.counter), IdSpace.COUNT_STRING_PART_WIDTH, '0');
